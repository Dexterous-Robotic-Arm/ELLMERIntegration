...
class TaskExecutor:
            ...
            for position_name, joint_angles in scan_positions:
                ...
                if not self.dry_run:
                    ...
                    if found_targets and interrupt_on_detection:
                        ...
                        approach_success = self._approach_detected_object_dynamically(found_targets[0])
                        if approach_success:
                            print(f"[ARC_SCAN] SUCCESS: Approached {found_targets[0]} from {position_name}")
                            return found_targets
                        else:
                            print(f"[ARC_SCAN] WARNING: Approach failed, continuing scan")
                else:
    ...
    def _approach_detected_object_dynamically(self, target_label: str) -> bool:
        """Approach object using dynamic detection orientation."""
        try:
            print(f"[DYNAMIC_APPROACH] ========== DYNAMIC APPROACH DEBUG ==========")
            print(f"[DYNAMIC_APPROACH] Target label: {target_label}")
            
            tcp_position, tcp_rpy = self.runner.get_current_position_and_rpy()
            if tcp_position is None or tcp_rpy is None:
                print(f"[DYNAMIC_APPROACH] ERROR: Could not get current position/orientation")
                print(f"[DYNAMIC_APPROACH] TCP Position: {tcp_position}")
                print(f"[DYNAMIC_APPROACH] TCP RPY: {tcp_rpy}")
                return False
            
            print(f"[DYNAMIC_APPROACH] Current TCP position: {tcp_position}")
            print(f"[DYNAMIC_APPROACH] Current TCP RPY: {tcp_rpy}")
            
            camera_coordinates = self.obj_index.wait_for(target_label, timeout=2.0)
            if camera_coordinates is None:
                print(f"[DYNAMIC_APPROACH] ERROR: Object '{target_label}' not detected")
                with self.obj_index._global_lock:
                    available_objects = list(self.obj_index.latest_mm.keys())
                print(f"[DYNAMIC_APPROACH] Available objects: {available_objects}")
                return False
            
            print(f"[DYNAMIC_APPROACH] Camera coordinates: {camera_coordinates}")
            print(f"[DYNAMIC_APPROACH] Camera frame: X={camera_coordinates[0]:.2f}mm (left/right)")
            print(f"[DYNAMIC_APPROACH] Camera frame: Y={camera_coordinates[1]:.2f}mm (up/down)")
            print(f"[DYNAMIC_APPROACH] Camera frame: Z={camera_coordinates[2]:.2f}mm (forward/back)")
            ...
            print(f"[DYNAMIC_APPROACH] Transformed robot base coords: {robot_base_coords}")
            print(f"[DYNAMIC_APPROACH] Robot frame: X={robot_base_coords[0]:.2f}mm (forward/back)")
            print(f"[DYNAMIC_APPROACH] Robot frame: Y={robot_base_coords[1]:.2f}mm (left/right)")
            print(f"[DYNAMIC_APPROACH] Robot frame: Z={robot_base_coords[2]:.2f}mm (up/down)")
            ...
            print(f"[DYNAMIC_APPROACH] Stored detection state:")
            print(f"[DYNAMIC_APPROACH]   Camera coords: {self.detection_state['last_target_coords']}")
            print(f"[DYNAMIC_APPROACH]   Robot coords: {self.detection_state['last_robot_coords']}")
            print(f"[DYNAMIC_APPROACH]   Detection RPY: {self.detection_state['last_detection_rpy']}")
            
            x, y, z = robot_base_coords[0], robot_base_coords[1], robot_base_coords[2]
            if abs(x) > 1000 or abs(y) > 1000 or z < 0 or z > 1000:
                print(f"[DYNAMIC_APPROACH] ERROR: Coordinates out of bounds")
                print(f"[DYNAMIC_APPROACH] X: {x:.1f}mm (limit: ±1000mm)")
                print(f"[DYNAMIC_APPROACH] Y: {y:.1f}mm (limit: ±1000mm)")
                print(f"[DYNAMIC_APPROACH] Z: {z:.1f}mm (limit: 0-1000mm)")
                return False
            ...
            print(f"[DYNAMIC_APPROACH] Approach calculation:")
            print(f"[DYNAMIC_APPROACH]   Base target: {robot_base_coords}")
            print(f"[DYNAMIC_APPROACH]   X offset: {x_offset}mm")
            print(f"[DYNAMIC_APPROACH]   Y offset: {y_offset}mm")
            print(f"[DYNAMIC_APPROACH]   Z hover: +{hover_height}mm")
            print(f"[DYNAMIC_APPROACH]   Final approach coords: {approach_coords}")
            
            print(f"[DYNAMIC_APPROACH] Movement parameters:")
            print(f"[DYNAMIC_APPROACH]   Target position: [{approach_coords[0]:.2f}, {approach_coords[1]:.2f}, {approach_coords[2]:.2f}]")
            print(f"[DYNAMIC_APPROACH]   Target orientation: [{tcp_rpy[0]:.2f}, {tcp_rpy[1]:.2f}, {tcp_rpy[2]:.2f}]")
            print(f"[DYNAMIC_APPROACH]   Speed: 150mm/s")
            
            print(f"[DYNAMIC_APPROACH] Executing movement...")
            ...
            print(f"[DYNAMIC_APPROACH] Movement result: {result}")
            ...
            if result == 0:
                ...
                if final_tcp:
                    ...
                    print(f"[DYNAMIC_APPROACH] Final TCP position: {final_tcp}")
                    print(f"[DYNAMIC_APPROACH] Movement error: {movement_error} (total: {total_error:.2f}mm)")
                
                print(f"[DYNAMIC_APPROACH] SUCCESS: Approached {target_label}")
                print(f"[DYNAMIC_APPROACH] =======================================")
                return True
            else:
                print(f"[DYNAMIC_APPROACH] ERROR: Movement failed with code {result}")
                print(f"[DYNAMIC_APPROACH] =======================================")
                return False
                
        except Exception as e:
            print(f"[DYNAMIC_APPROACH] ERROR: Exception during approach: {e}")
            print(f"[DYNAMIC_APPROACH] =======================================")
            return False
        ...
        for i, step in enumerate(steps_list, 1):
            ...
            try:
                if act == "MOVE_TO_NAMED":
                    ...
                    print(f"[APPROACH] ========== APPROACH OBJECT DEBUG ==========")
                    print(f"[APPROACH] Target label: {target_label}")
                    print(f"[APPROACH] Approach distance: {approach_distance}mm")
                    
                    if not self.dry_run:
                        if self.detection_state['last_detection_rpy'] and self.detection_state['last_robot_coords']:
                            print(f"[APPROACH] Using stored detection state for {target_label}")
                            robot_base_coords = self.detection_state['last_robot_coords'].copy()
                            detection_rpy = self.detection_state['last_detection_rpy'].copy()
                            
                            print(f"[APPROACH] Stored robot coords: {robot_base_coords}")
                            print(f"[APPROACH] Stored detection RPY: {detection_rpy}")
                            ...
                            print(f"[APPROACH] Calculated approach coords: {approach_coords}")
                            print(f"[APPROACH] Coordinate breakdown:")
                            print(f"[APPROACH]   X: {robot_base_coords[0]:.2f} → {approach_coords[0]:.2f} (no change)")
                            print(f"[APPROACH]   Y: {robot_base_coords[1]:.2f} → {approach_coords[1]:.2f} (offset: -40mm)")
                            print(f"[APPROACH]   Z: {robot_base_coords[2]:.2f} → {approach_coords[2]:.2f} (hover: +{approach_distance}mm)")
                        else:
                            print(f"[APPROACH] No stored detection state, calculating fresh coordinates")
                            tcp_position, tcp_rpy = self.runner.get_current_position_and_rpy()
                            if tcp_position is None or tcp_rpy is None:
                                print(f"[ERROR] Could not get robot position for {target_label}")
                                print(f"[ERROR] TCP Position: {tcp_position}")
                                print(f"[ERROR] TCP RPY: {tcp_rpy}")
                                continue
                            ...
                            if not target_coords:
                                print(f"[APPROACH] ERROR: Could not get coordinates for {target_label}")
                                with self.obj_index._global_lock:
                                    available = list(self.obj_index.latest_mm.keys())
                                print(f"[APPROACH] Available objects: {available}")
                                continue
                            
                            print(f"[APPROACH] Fresh camera coordinates: {target_coords}")
                        ...
                        print(f"[APPROACH] Final movement parameters:")
                        print(f"[APPROACH]   Position: [{approach_coords[0]:.2f}, {approach_coords[1]:.2f}, {approach_coords[2]:.2f}]")
                        print(f"[APPROACH]   Orientation: [{detection_rpy[0]:.2f}, {detection_rpy[1]:.2f}, {detection_rpy[2]:.2f}]")
                        print(f"[APPROACH]   Speed: {step.get('speed', 100)}mm/s")
                        
                        if self._is_coordinate_safe(approach_coords):
                            try:
                                print(f"[APPROACH] Executing movement command...")
                                result = self.runner.arm.set_position(
                                    x=approach_coords[0],
                                    y=approach_coords[1], 
                                    z=approach_coords[2],
                                ...
                                print(f"[APPROACH] Movement result code: {result}")
                                
                                if result == 0:
                                    final_pos = self.runner.get_current_position()
                                    if final_pos:
                                        error = [abs(final_pos[i] - approach_coords[i]) for i in range(3)]
                                        print(f"[APPROACH] Final position: {final_pos}")
                                        print(f"[APPROACH] Position error: {error} (total: {sum(error):.2f}mm)")
                                    print(f"[APPROACH] SUCCESS: Approached {target_label}")
                                else:
                                    print(f"[APPROACH] ERROR: Failed to approach {target_label}, error: {result}")
                                    
                            except Exception as e:
                                print(f"[APPROACH] ERROR: Exception during movement: {e}")
                        else:
                            print(f"[APPROACH] ERROR: Approach coordinates failed safety check")
                            print(f"[APPROACH] Unsafe coordinates: {approach_coords}")
                    else:
                        print(f"[APPROACH] DRY RUN: Would approach {target_label}")
