...
class TaskExecutor:
    def __init__(self, arm_ip: str, world_yaml: str = None, sim: bool = False, dry_run: bool = False):
        ...
        # Runtime replanning guard
        self._runtime_replan_used = False
                    ...
                    if not self.dry_run:
                        try:
                            ...
                            if obj is None:
                                ...
                                # Trigger runtime replan if possible
                                if not self._runtime_replan_used:
                                    self._attempt_runtime_replan(i, act, f"object_not_detected:{target_label}", plan)
                                    return
                                continue
                            ...
                            # If failure due to missing object or timeout, replan
                            if not self._runtime_replan_used:
                                self._attempt_runtime_replan(i, act, f"exception:{e}", plan)
                                return
                            continue
                    else:
    ...
    def _attempt_runtime_replan(self, failed_step_idx: int, failed_action: str, reason: str, last_plan: dict):
        """Attempt to trigger runtime replanning by calling back into the RAG planner via main planner path.

        We construct a brief diagnostic string and invoke the planner with the same user goal but
        augmented context. Then we immediately execute the new plan once, and prevent further replans
        in this execution to avoid loops.
        """
        try:
            from robot_control.rag_system.true_rag_planner import TrueRAGPlanner
        except Exception as e:
            print(f"[Replan] Cannot import RAG planner: {e}")
            return

        try:
            self._runtime_replan_used = True
            user_goal = last_plan.get('goal', 'previous goal')
            diagnostic = f"Runtime failure at step {failed_step_idx} ({failed_action}): {reason}. " \
                         f"Objects seen: {list(self.obj_index.latest_mm.keys())}"
            print(f"[Replan] Triggering RAG replanning: {diagnostic}")

            rag_planner = TrueRAGPlanner(robot_controller=self, vision_system=self.obj_index, config_path="config/", db_path="data/vector_db")
            # Augment the query with diagnostic to give the LLM more context
            augmented_query = f"{user_goal}. Note: {diagnostic}. Please adapt the plan (consider scanning or different target/pose)."
            new_plan = rag_planner.plan_with_rag(augmented_query)
            print("[Replan] Executing new plan generated by RAG...")
            self.execute(new_plan)
        except Exception as e:
            print(f"[Replan] Replanning failed: {e}")
